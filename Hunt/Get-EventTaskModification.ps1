Function Get-EventTaskModification {
<#
.SYNOPSIS
    Get scheduled task modification events from Security logs on a remote computer.
    Privileges required: high

    Author: Timothée MENOCHET (@_tmenochet)

.DESCRIPTION
    Get-EventTaskModification queries remote host for scheduled task modification events.
...
.EXAMPLE
    PS C:\> Get-EventTaskModification -SubjectSID S-1-5-18 -InvertLogic -ComputerName SRV.ADATUM.CORP -Credential ADATUM\Administrator
#>
    ...
    if ($SubjectSID) {
        if ($InvertLogic) {
            $filterXPath = "*[System[EventID=4702] and EventData[Data[@Name='SubjectUserSid'] !='$SubjectSID']]"
        }
        else {
            $filterXPath = "*[System[EventID=4702] and EventData[Data[@Name='SubjectUserSid'] and (Data='$SubjectSID')]]"
        }
    }
    else {
        $filterXPath = "*[System[EventID=4702]]"
    }
    $params = @{
